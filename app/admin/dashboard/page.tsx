...
export default function AdminDashboard() {
  ...
  // Keep a ref to the latest fetchSubmissions so the realtime callback never goes stale
  const fetchSubmissionsRef = useRef<() => void>(() => { })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Always call the latest version to avoid stale closures
  ...
  // Keep ref up-to-date and re-fetch whenever filters or fetchSubmissions changes
  useEffect(() => {
                ...
                {loading ? (
                  ...
                  submissions.map((submission) => (
                    <TableRow key={submission.id} className="border-b-white/10 hover:bg-white/5">
                      ...
                      <TableCell>
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${submission.status === 'pending' ? 'bg-yellow-400 text-yellow-900' :
                      ...
                      <TableCell>
                        ...
                        {(submission.status === 'approved' || submission.status === 'rejected') && (
                          <div className="flex items-center space-x-2">
                            <span className={`flex items-center ${submission.status === 'approved' ? 'text-green-400' : 'text-red-400'
                              }`}>
